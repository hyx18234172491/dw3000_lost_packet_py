...
def log_ranging(link_uri, log_cfg_name='TSranging', log_var={}, log_save_path=None, period_in_ms=100,
                keep_time_in_s=5):
...
def takeoffAndlog_ranging(link_uri, log_cfg_name='TSranging', log_var={}, log_save_path=None, period_in_ms=100,
                keep_time_in_s=5):
    cflib.crtp.init_drivers(enable_debug_driver=False)
    log_data = pd.DataFrame(columns=log_var.keys())
    ...
    with SyncCrazyflie(link_uri=link_uri, cf=Crazyflie(rw_cache="./cache")) as scf:
        # 起飞
        # scf.cf.param.set_value('relative_ctrl.keepFlying', 1)
        # 日志
        log_cfg = LogConfig(name=log_cfg_name, period_in_ms=period_in_ms)
        for log_var_name, log_var_type in log_var.items():
            log_cfg.add_variable(log_var_name, log_var_type)
...
def log_ranging_1(link_uri, log_cfg_name='TSranging', log_var={}, log_save_path=None, period_in_ms=100,
                  keep_time_in_s=5):
    cflib.crtp.init_drivers(enable_debug_driver=False)
    log_data = pd.DataFrame(columns=log_var.keys())
    ...
    with SyncCrazyflie(link_uri=link_uri, cf=Crazyflie(rw_cache="./cache")) as scf:
        ...
        with SyncLogger(scf, log_cfg) as logger:
            for log_entry in logger:
                ...
                if (time.time()-start_time > keep_time_in_s):
                    break
